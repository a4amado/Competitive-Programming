...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        # detect circle in a direct graph
        # convert the list of edges to a adjacency List
        adjs = [[] for x in range(numCourses)] 
        for course, prereq in prerequisites:
            adjs[prereq].append(course)
        
        # deletect circl in a adjs
        visited = [False] * numCourses

        def dfs(idx: int, path: set):
            neighbours = adjs[idx]
...
class Solution:
    def canFinish(self, numCourses: int, prerequisites: List[List[int]]) -> bool:
        ...
        for i in range(numCourses):
            if dfs(i, set()):
                return False  # Cannot finish if cycle detected
